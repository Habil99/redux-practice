import React, { useState } from 'react'
import './Login.css'
import { useDispatch, useSelector } from 'react-redux'
import { setUser, authGoogleUser } from '../features/userSlice'
import { LoadingOutlined } from '@ant-design/icons'
import { Redirect } from 'react-router'

const Login = () => {
    const [email, setEmail] = useState('')
    const [password, setPassword] = useState('')
    const user = useSelector(state => state.user.user)
    const status = useSelector(state => state.user.status)
    const message = useSelector(state => state.user.errorMessage)
    const dispatch = useDispatch()

    const authUser = () => {
        dispatch(setUser({ email, password }))
    }

    const authWithGoogle = () => {
        dispatch(authGoogleUser())
    }

    return (
        <>
            <div className="card">
                <form onSubmit={(e) => e.preventDefault()}>
                    <h2 className="title"> Log in</h2>
                    <p className="subtitle">Don't have an account? <a href="https"> sign Up</a></p>
                    <div className="social-login">
                        <button className="google-btn" onClick={authWithGoogle}>
                            <img alt="Google" src="data:image/svg+xml;base64," />
                            <p className="btn-text">Sign in with Google</p>
                        </button>
                        <button className="fb-btn">
                            <img alt="FB" src="data:image/svg+xml;base64," />
                        </button>
                    </div>
                    <p className="or"><span>or</span></p>
                    <div className="email-login">
                        <label htmlFor="email"> <b>Email</b></label>
                        <input type="text" onInput={e => setEmail(e.target.value)} placeholder="Enter Email" name="uname" required />
                        <label htmlFor="psw"><b>Password</b></label>
                        <input type="password" onInput={e => setPassword(e.target.value)} placeholder="Enter Password" name="psw" required />
                    </div>
                    {message !== null && <div className="alert-error">{message}</div>}
                    <button onClick={authUser} disabled={status === 'loading' ? 'disabled' : null} className="cta-btn" type="submit">{status === 'loading' ? <LoadingOutlined /> : 'Log In'}</button>
                    <a className="forget-pass" href="https">Forgot password?</a>
                </form>
            </div>
            {user !== null  && <Redirect to="/" />}
        </>
    )
}

export default Login
